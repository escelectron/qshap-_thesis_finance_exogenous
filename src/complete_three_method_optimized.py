...
Optimized Quantum Training (Gradient-based Adam Optimizer)
- Method 1: SHAP (correlational, XGBoost)
- Method 2: SHAP+ (causal, XGBoost)
- Method 3: Q-SHAP+ (causal, Quantum)
"""
import os
import numpy as np
import pandas as pd
...
# --- Global QuantumModel class (needed for joblib.load) ---
class QuantumModel:
    def __init__(self, weights, quantum_circuit=None):
        self.weights = weights
        self.quantum_circuit = quantum_circuit  # circuit reference injected after load
...
# Try to import PennyLane for quantum implementation
try:
...
df = pd.read_excel("../data/default_of_credit_card_clients.xlsx",
...
# ============================================================================
# STEP 5: TRAIN OR LOAD QUANTUM MODEL (Optimized with qml.grad)
# ============================================================================
print("\n[STEP 5] Quantum Model (Adam optimizer with caching)...")

# --- Global class defined at top of file (required for joblib) ---
class QuantumModel:
    def __init__(self, weights, quantum_circuit=None):
        self.weights = weights
...
# --- Device + quantum circuit definition ---
...
# ============================================================================
# STEP 7–9: EXPLAINABILITY METHODS
# ============================================================================
...
# ============================================================================
# STEP 13: VALIDATION METRICS & FEATURE IMPORTANCE COMPARISON
# ============================================================================
...
val_metrics = pd.DataFrame({
    "Method": ["SHAP", "SHAP+", "Q-SHAP+"],
    "Accuracy": [
        accuracy_score(y_test, xgb_model.predict(X_test)),  # same model for SHAP & SHAP+
...
# ============================================================================
# STEP 13: INTERPRETABILITY METRICS COMPARISON (with error bars)
# ============================================================================
print("\n[STEP 13] Plotting interpretability metrics comparison (with error bars)...")
...
# ============================================================================
# STEP 14: SAVE DATA FOR ALL CHARTS & APPENDIX TABLES
# ============================================================================
print("\n[STEP 14] Saving chart data and 54-profile appendix tables...")

# === 1️⃣ Validation Metrics CSV (for validation_metrics_comparison.png) ===
...
# === 2️⃣ Interpretability Metrics CSV (for interpretability_metrics_comparison.png) ===
...
# === 3️⃣ Feature Importance CSV (for validation_feature_importance_comparison.png) ===
importance_path = "../results/feature_importance_data.csv"
importance_df.to_csv(importance_path, index=False)
print(f"✓ Saved feature importance → {importance_path}")

# === 4️⃣ Composite Three-Method Comparison CSV (already exists, rewrite for consistency) ===
...
# === 5️⃣ Detailed Appendix: 54 Profiles with SHAP / SHAP+ / Q-SHAP+ Attributions ===
appendix_df = X_sample.copy()
appendix_df["Default"] = y_sample.values

# Flatten attribution arrays into labeled columns
...
# === 6️⃣ Console preview of first few rows (pretty format for appendix printout) ===
pd.set_option("display.max_columns", None)
pd.set_option("display.width", 160)
print("\nPreview of Appendix Table (first 5 of 54 profiles):")
#print(appendix_df.head(5).round(3).to_string(index=False))

# ============================================================================
# STEP 15: CREATE THESIS TABLES (Table 4 & Table 5)
# ============================================================================
print("\n[STEP 15] Generating thesis-style tables (Table 4 & Table 5)...")

# === Table 4: Feature Attribution Comparison (mean absolute values) ===
...
print("\nInterpretation:")
print("Q-SHAP⁺ redistributes attribution weight, revealing alternate causal features (e.g., MARRIAGE).")

# === Table 5: Quality / Interpretability Metrics Comparison ===
...
# === Optional composite scores (used for hypothesis testing) ===
composite_scores = {
...
for k, v in composite_scores.items():
    print(f"  {k:7s} → {v:.2f}")
print(f"\nΔ = X̄(Q-SHAP⁺) − X̄(SHAP) = {delta:.2f}  (used in hypothesis test)")

# ============================================================================
# STEP 16: QUANTUM PHENOMENA DETECTION (Entanglement & Interference)
# ============================================================================
print("\n[STEP 16] Computing quantum phenomena (entanglement, interference)...")

import seaborn as sns

# --- Compute feature entanglement (pairwise correlation of Q-SHAP⁺ attributions)
...
# Save numeric entanglement data
...
# --- Plot heatmap of entanglement strength
...
# --- Compute summary quantum effects
...
# ============================================================================
# STEP 17: PRINT AND SAVE INTERPRETABILITY MATRIX (54 PROFILES)
# ============================================================================
print("\n[STEP 17] Generating interpretability matrix for all 54 profiles...")

# Compute per-profile composite interpretability score
def compute_composite(m):
    """Weighted composite interpretability score per profile"""
    return (
        0.30 * m[:, 0] +  # Faithfulness
        0.30 * m[:, 1] +  # Stability
        0.20 * m[:, 2] +  # Responsiveness
        0.20 * (m[:, 3] / 5 * 100)  # Clarity normalized to %
    )
...
# Build DataFrame for 54 profiles
...
# Print summary table (first few rows for display)
print("Interpretability Matrix (54 Profiles)\n")
print(interpret_df.head(10).to_string(index=False))  # Print first 10 for readability
print("\n... (Full matrix of 54 profiles saved to CSV)")

# ============================================================================
# STEP 17: AGGREGATE INTERPRETABILITY MATRIX (54 PROFILES, 4 COLUMNS)
# ============================================================================
print("\n[STEP 17] Aggregating interpretability metrics into single mean score per profile...")

# Compute per-profile composite interpretability score (same weighting as before)
def compute_composite(m):
    """Weighted composite interpretability score per profile"""
    return (
        0.30 * m[:, 0] +  # Faithfulness
        0.30 * m[:, 1] +  # Stability
        0.20 * m[:, 2] +  # Responsiveness
        0.20 * (m[:, 3] / 5 * 100)  # Normalize clarity to percentage
    )

# Compute averaged interpretability score for each profile
shap_comp_all = compute_composite(shap_metrics)
shapplus_comp_all = compute_composite(shapplus_metrics)
qshap_comp_all = compute_composite(qshap_metrics)
...
# Save to CSV
matrix_path = "../results/interpretability_matrix_54_profiles_simplified.csv"
interpret_simple_df.to_csv(matrix_path, index=False)
print(f"✓ Saved simplified interpretability matrix → {matrix_path}\n")

# Print first few rows for inspection
print("Simplified Interpretability Matrix (54 Profiles)\n")
print(interpret_simple_df.head(10).to_string(index=False))
print("\n... (Full 54-profile matrix saved to CSV)")

# ============================================================================
# STEP 18: TWO-WAY FEATURE ATTRIBUTION COMPARISON (SHAP vs Q-SHAP+)
# ============================================================================
print("\n[STEP 18] Plotting two-way feature attribution comparison: SHAP vs Q-SHAP+ ...")

# Compute mean absolute attributions for both methods
shap_mean = np.mean(np.abs(shap_values), axis=0)
qshap_mean = np.mean(np.abs(qshap_values), axis=0)

# Save numeric data for reproducibility
...
print(f"✓ Saved feature attribution data → {attr_comp_path}")
...
plt.title("Feature Attribution Comparison", fontsize=13, fontweight="bold")
plt.legend(frameon=False, fontsize=10)
...
# ============================================================================
# STEP 19: TWO-WAY INTERPRETABILITY METRICS COMPARISON (SHAP vs Q-SHAP+)
# ============================================================================
print("\n[STEP 19] Plotting interpretability metrics: SHAP vs Q-SHAP+ ...")
...
print(f"✓ Saved numeric results → {two_way_path}")
...
bars2 = plt.bar(x + width/2, means_qshap, width,
                ...
                label="Q-SHAP+ (Quantum)", color="#9b59b6", edgecolor="black")
